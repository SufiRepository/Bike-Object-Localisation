...
import ai.certifai.solution.object_detection.BikeDetector.dataHelpers.NonMaxSuppression;
 ...
 * Pascal VOC
 */
public class BikeImageDetector {
    private static final Logger log = LoggerFactory
            .getLogger(ai.certifai.solution.object_detection.ActorsDetector.tinyyolo.BikeDetector.class);
    private static ComputationGraph model;

    // parameters matching the pretrained YOLO model
    ...
    private static String modelFilename = new File(".").getAbsolutePath() + "/generated-models/BikeDetector_yolov2.zip";

    public static void main(String[] args) throws Exception {

        // Directory for Bike train and test datasets
        File trainDir = new ClassPathResource("bicycle/train").getFile();
        File testDir = new ClassPathResource("bicycle/test").getFile();
        ...
        if (new File(modelFilename).exists()) {
            ...
            /* STEP 1: Transfer Learning steps - Load YOLO prebuilt model. */
    ...
    private static ComputationGraph getNewComputationGraph(ComputationGraph pretrained, INDArray priors,
            FineTuneConfiguration fineTuneConf) {
        ComputationGraph _ComputationGraph = new TransferLearning.GraphBuilder(pretrained)
                .fineTuneConfiguration(fineTuneConf).removeVertexKeepConnections("conv2d_9")
    ...
    private static FineTuneConfiguration getFineTuneConfiguration() {
        ...
        FineTuneConfiguration _FineTuneConfiguration = new FineTuneConfiguration.Builder().seed(seed)
    ...
    private static void drawBoxes(List<DetectedObject> objects, int w, int h, Mat image) {
        for (DetectedObject obj : objects) {
            ...
            putText(image, label + " - " + String.format("%.2f", proba * 100) + "%", new Point((x1 + 2), (y1 + y2) / 2),
                    FONT_HERSHEY_DUPLEX, 0.5, Scalar.RED);
        }
    }
}